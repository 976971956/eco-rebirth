...
| `scripts/developer_tuning.gd` | Developer-mode parameter metadata, defaults, safe ranges, sanitization and JSON interchange |
...
- Keep evolution-energy rain active through the whole match: every 60 seconds levels 1–10 add 10–100 packs, older packs may cross waves, and an active cap of 15–150 prevents unbounded mobile/Web nodes. During final collapse, retire packs outside the safe radius and land new waves inside it. Player and AI share absorption, interruption, reward, and risk rules.
...
- Keep normal settings focused on audio, graphics, tutorial/campaign reset and an explicit developer-mode entry. Developer tuning must use a sanitized whitelist, apply shared animal multipliers to player and AI, be ignored by automation, and permanently mark an affected run as non-progressing. Free mode and the easy/adventure/hard run-difficulty selector belong on the home page. Lock difficulty when a world starts; preserve difficulty and developer preferences when campaign progress resets.
- Keep the V1.49 all-species art path mobile/Web safe: all 30 species use versioned Hero/Mobile Blender GLBs and automatic body/detail ranges. The non-wolf V5 appearance layer must use per-species torso/head/muzzle proportions, thin authored ears, restrained facial features, tapered feet/hooves/claws/talons, and flush coat/scale markings; never restore the old giant eyes, sphere ears or protruding pattern blobs. Rabbit plus the 24 non-wolf ground species retain the V4 22-bone contract with four `Leg → Lower → Paw` chains, an articulated jaw, and a two-bone tail; owl/eagle use 14-bone three-stage wings; snake/crocodile use 10/22-bone axial rigs, with the crocodile also carrying four low, splayed three-stage limbs. The CC0/AI-textured cinematic wolf remains unchanged as a 32-bone source rig with articulated paws, jaw, eyes and a four-segment tail. Never assume a Blender Euler component is a hinge axis: imported actions must rotate around the converted rest-bone transverse axis, not the bone's longitudinal Y. Paw motion or a local quaternion delta alone is insufficient. The release gate samples true joint-origin chain angles, paw travel and paw orientation for every V4 ground species, wing-tip travel/secondary-wing fold/primary-feather rotation for both birds, and rear-spine/chest/tail-tip wave propagation for both long-body species. The wolf keeps its stricter trot/sprint chain-angle margins. Every `OrganicBodyV2` must contain exactly one connected source torso island; UV/tangent seam splits and V5 flush multi-material surfaces may use the validated `SourceConnected` node contract. Silhouette attachments may be separate, while face, hoof, feather and small marking meshes must remain independently distance-cullable. Every model keeps the eight core baked actions and family-specific sockets. `EcoActor` must select and play imported `AnimationPlayer` actions; standardized Godot Euler poses are fallback-only when an action is absent. AI and low-quality players always use Mobile, with the active 30-species Mobile total at or below 140,000 vertices (V1.49 baseline: 125,344). Missing V2 assets may fall back to V1. Collision, stats, AI and combat-event ownership remain in `EcoActor`; imported models rotate at the catalog root so visuals and sockets follow Godot -Z forward.
...
- Three run difficulties, AI intelligence gradients, fairness limits, persistence and acceptance metrics: `docs/20_三档难度与AI智能设计.md`
- Developer-mode parameters, save isolation, live/next-run semantics and mobile UI: `docs/21_开发者模式与调参设计.md`
