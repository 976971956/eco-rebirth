...
REPORT_ROOT="${1:-/tmp/eco-rebirth-v139-rc}"
...
if [ "${ECO_RC_SOAK:-0}" = "1" ]; then
	run_godot_check soak-level-1 -- --batch-sim=2 --batch-level=1 --world-seed=138001 --report-dir="$REPORT_ROOT/soak"
	run_godot_check soak-level-10 -- --batch-sim=2 --batch-level=10 --world-seed=138010 --report-dir="$REPORT_ROOT/soak"
