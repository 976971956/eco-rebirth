...
V1.50 启动逐种外部写实替换；当前雪兔、赤狐、林鹿、青环蛇、棕熊和獠牙野猪已完成经审计的 CC0 来源记录、物种化 UV/PBR、项目骨架契约与八态烘焙动作。棕熊与野猪不直接采用审计后不满足当前质量线的旧拓扑，而是建立连续四足毛皮网格与跨关节权重。构建器使用 `--disable-autoexec` 审计源 `.blend`，并生成 Hero/Mobile 双档；`./tools/build_realistic_vertical_slice.sh` 会先重建这六种和灰狼，再重建尚未替换的回退物种。当前 30 种 Mobile 总顶点为 128,308 / 140,000，候选运行 `./tools/build_release_candidate.sh /tmp/eco-rebirth-v150-rc`。
