...
V1.56 将深水屏息条改到屏幕中下方、底部技能状态栏上方，避免遮住玩家自身。HUD 只在实际水深超过 0.58m 且超过当前物种安全涉水线时显示；中水、安全涉水和飞行状态隐藏。紧凑手机 HUD 自动使用 360×82 面板，与右侧按钮和底部技能栏保留间距，并且不截获摇杆或其他触摸输入。候选运行 `./tools/build_release_candidate.sh /tmp/eco-rebirth-v156-rc`。
