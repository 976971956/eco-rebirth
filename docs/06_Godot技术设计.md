...
- 触控动作保持在右下两列，攻击、主动技能、进食、冲刺之间保留至少 12 逻辑像素间隔；动态摇杆仍可在左侧下半区任意位置唤醒。四键使用程序矢量图标作为主要识别，技能按钮按 30 种物种映射 27 类动作语义并用外环显示冷却，文字只补充技能名和状态；普通可选控件至少 52 逻辑像素高，主要动作按钮保持 96–150 逻辑像素。
- 动态摇杆锁定首个有效触点的编号与输入来源；首指未释放前，第二触点和移动平台生成的模拟鼠标事件只能操作右侧按钮，不能接管、重定位或取消摇杆。
...
- V1.66 起，每次建立世界后进入独立 `intro` 状态：物种攻略简报不设自动计时，生态树暂停，移动与战斗触控层隐藏；只有点击至少 52 逻辑像素高的“关闭并开始”才发出信号返回 `playing`。自动烟测/性能参数显式跳过人工确认，避免 CI 挂起。
- 安全区域每 0.45 秒复核一次，以覆盖不改变视口尺寸的 180 度横屏翻转，同时也监听视口尺寸变化。
- 玩家升级后的生命与耐力上限由 HUD 更新循环持续同步，而不是只在绑定角色时赋值；状态摘要统一显示攻击、速度、护甲与耐力恢复。
...
- `SAVE_VERSION = 4`；旧版本读取后补齐空的 `discovered_species`、`species_records` 和 `recent_runs`，不覆盖声音、画质、教程或战役进度。
- `discovered_species` 仅接受当前 30 种目录 ID，并按目录顺序保存；同局重复物种只登记一次。
- `species_records` 按物种聚合出战、胜利、最高关卡、最佳存活、最高局内等级和单局最佳战绩；所有外部数值在载入时限制到合法范围。
- `recent_runs` 最多保留 10 局，记录模式、关卡、世界种子、死因、成长、战斗、生存与新发现摘要，不保存逐帧 Actor 状态。
- 图鉴和复盘数据从不写回 `SpeciesCatalog`，也不参与局内属性公式。
