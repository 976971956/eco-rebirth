...
当前仓库已包含一个可以直接运行的完整生态对战原型：四区域程序化低多边形地图、14 种按关卡解锁的随机玩家物种、10–100 个生态个体、AI 捕食/逃亡/预测绕障、耐力与饱腹、六种野外食物、尸体进食、击杀与生态助攻经验、八级局内成长、精确生命数值、差异化生态技能、死亡轮回、威胁成长、胜负结算，以及键鼠和触屏操作。
...
11. [关卡物种扩展设计](docs/09_关卡物种扩展设计.md)：14 种当前实现、逐关解锁规则、数值定位与后续 22 种扩展顺序。
...
| 首发内容目标 | 当前可玩 14 种；下一阶段扩至 22 种，正式版目标 30–40 种 | “300 种”作为长期扩展目标 |

## 文档使用方式

- 策划修改规则时，先改对应系统文档，再同步 `08_数据字典与初始平衡.md`。
- Godot 内的关卡、物种和技能数据使用自定义 `Resource` 配置，不把平衡数值硬编码进场景脚本。
