...
- **简单**：AI 仍保留完整生态行为，但观察、记忆、协作与战斗压力较低，适合学习物种习性和手机操作；
- **冒险**：推荐的正式平衡，AI 完整使用围猎、避险、经验争夺、技能时机和环境反制；
- **困难**：AI 更快重新评估目标，扩大有限感知与同类情报距离，延长追踪记忆，并附加克制的小幅战斗压力。

难度在首页选择并写入存档，进入一局后锁定。任何难度都不允许 AI 获得玩家专属仇恨或全图坐标；自由模式同样可选择难度，但继续保持世界威胁为零。详细参数与公平边界见[三档难度与 AI 智能设计](20_三档难度与AI智能设计.md)。

### 2.3 开发者模式

游戏设置中提供独立的开发者模式，可在 Web、Android、iOS 和桌面横屏界面调整动物、成长资源、世界和 AI 四类运行参数。调试参数使用白名单和安全范围，玩家与 AI 共用动物属性；启用后的当前局不推进战役、威胁、图鉴战绩或最近轮回。详细规则见[开发者模式与运行时调参设计](21_开发者模式与调参设计.md)。
...
读取当前本能与附近威胁 → 在两条可选完成路线中选择觅食、迁徙、反制或借力 → 冲刺/攻击/技能 → 消耗耐力 → 隐藏踪迹或脱离恢复 → 推进本能并重新判断。

### 3.2 3 分钟循环

发现一场 AI 冲突或生态热点 → 读取迁徙风险、历史足迹与危险记忆 → 选择介入、埋伏、追踪或绕行 → 制造击杀或等待争食 → 进食恢复 → 改变区域食物链 → 向下一处资源、迁徙信号或声音移动。
...
6. 展示约 8 秒“生态简报”：物种名称、体型、食性、主动技能、三段本能与核心反制路线。
7. 所有玩家和 AI 共享 8 秒生态建立保护；攻击或释放技能会提前解除自身保护，之后生态正式运行。
...
- 可逆袭、地形反制和可借力第三方的短状态提示。
- 当前生态热点的名称、八方向、距离、剩余时间、迁徙数、猎手数与风险等级。
- 最近可追踪猎物足迹或危险死亡地点的方向、距离和经过时间。
- 当前三段生态本能、阶段编号和两条可选完成进度。
