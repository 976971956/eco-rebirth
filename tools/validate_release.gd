...
func _validate_skill_plan_contract() -> void:
	...
	_expect(Catalog.SKILL_SIGNATURES.size() == Catalog.ORDER.size(), "招牌机制与使用代价没有覆盖全部30种动物")
	for species_id in Catalog.ORDER:
		...
		_expect(str(plan.get("mechanic", "")).length() >= 8 and str(plan.get("commitment", "")).length() >= 8, "%s 缺少可读的招牌机制或使用代价" % Catalog.display_name(species_id))
		_expect(Catalog.skill_plan_description(species_id).contains("招牌机制") and Catalog.skill_plan_description(species_id).contains("使用代价") and Catalog.skill_plan_description(species_id).contains("强化条件") and Catalog.skill_plan_description(species_id).contains("敌方反制"), "%s 技能方案没有进入统一展示文本" % Catalog.display_name(species_id))
	var actor_source := FileAccess.get_file_as_string("res://scripts/eco_actor.gd")
	...
	_expect(actor_source.contains("func is_skill_attack_immune") and actor_source.contains("PORCUPINE_REFLECT_RATIO := 0.50") and actor_source.contains("target.is_skill_attack_immune()"), "豪猪刺球无敌、50%反伤或AI等待反制没有进入共享战斗流程")
...
func _validate_multitouch_joystick_contract() -> void:
	...
	_expect(joystick._real_touch_blocked(), "真实摇杆触点开始后没有进入鼠标竞态屏蔽窗口")
...
func _validate_experience_drop_and_final_tracking_contract() -> void:
	...
	var contested_utility := ActorScript.experience_pack_utility(32, 2.4, 16.0, 3, 0.85, 40.0, false, 3, 0.72)
	var starving_utility := ActorScript.experience_pack_utility(32, 2.4, 16.0, 3, 0.85, 40.0, false, 0, 0.0, 0.94)
	var expiring_utility := ActorScript.experience_pack_utility(32, 2.4, 28.0, 3, 0.85, 40.0, false, 0, 0.0, 0.2, 3.0, 5.0)
	_expect(safe_utility > contested_utility and safe_utility > starving_utility and expiring_utility < 0.0, "AI 没有按竞争人数、饥饿与经验包消散时间分流")
	_expect(ActorScript.experience_contest_state(0, 0.2, 0.0) == "solo" and ActorScript.experience_contest_state(2, 0.68, 0.30) == "leading" and ActorScript.experience_contest_state(1, 0.18, 0.60) == "trailing", "玩家经验吸收条无法区分单独、领先与落后")
	_expect(WorldScript.EXPERIENCE_PACK_CLEAR_RADIUS >= 1.0 and WorldScript.EXPERIENCE_PACK_MIN_SPACING >= 3.0, "经验包落点没有为大型动物保留净空或包间距")
	var actor_source := FileAccess.get_file_as_string("res://scripts/eco_actor.gd")
