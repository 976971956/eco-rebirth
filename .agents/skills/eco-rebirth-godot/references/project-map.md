...
- Keep the V1.45 all-species art path mobile/Web safe: all 30 species use versioned Hero/Mobile Blender GLBs and automatic body/detail ranges. Most ground species use 16-bone bodies; the CC0/AI-textured cinematic wolf sample uses a 32-bone source rig with articulated paws, jaw, eyes and a four-segment tail. Its adult-wolf mass correction may deform shoulders, ribcage, neck and haunches, but must preserve the face, lower legs and gameplay collision; locomotion must convert the shared cross-body hinge axis through each source bone's rest matrix instead of assuming local Euler X. Owl/eagle use 10-bone two-stage wings; snake/crocodile use 8/12-bone long-body rigs. Every `OrganicBodyV2` must contain exactly one connected source torso island; UV/tangent seam splits may use the validated `SourceConnected` node contract. Silhouette attachments may be separate, while face, hoof, feather and small marking meshes must remain independently distance-cullable. Every model keeps the eight core baked actions and family-specific sockets. `EcoActor` must select and play imported `AnimationPlayer` actions; never apply standardized Godot Euler poses directly to Blender-converted rest axes. Manual pose controllers are fallback-only when an action is absent. AI and low-quality players always use Mobile, with the active 30-species Mobile total at or below 140,000 vertices (V1.45 baseline: 121,264). Missing V2 assets may fall back to V1. Collision, stats, AI and combat-event ownership remain in `EcoActor`; imported models rotate at the catalog root so visuals and sockets follow Godot -Z forward.
